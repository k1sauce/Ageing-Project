...
# create vector of probes to remove blood composition correlated probes 
# http://bioconductor.org/packages/release/data/experiment/html/FlowSorted.Blood.450k.html
# See "Accounting for cellular heterogeneity is critical in epigenome-wide association studies", AE Jaffe, Genome Biol. 2014; 15(2): R31.
# also http://dx.plos.org/10.1371/journal.pone.0041361 "Differential DNA Methylation in Purified Human Blood Cells: Implications for Cell Lineage and Studies on Disease Susceptibility"
# Note all samples used to calculate blood composition correlations are from six male samples. Results may not apply to females.
biocLite("FlowSorted.Blood.450k")
library(FlowSorted.Blood.450k)
data("FlowSorted.Blood.450k")
compTable <- FlowSorted.Blood.450k.compTable
correlatedprobesbloodremove <- subset(compTable, compTable$p.value < 0.05)
correlatedprobesbloodremove <- rownames(correlatedprobesbloodremove)
save(correlatedprobesbloodremove, file = "correlatedprobesbloodremove.r")
